...
class Terrain
{
  ...
  private:
    ...
    void InitDivideQuad(QuadTree **node, const float cx, const float cz, const float radius, const MeshData &m);
  ...
  public:
    void Initialize(ID3D12Device *device, ID3D12GraphicsCommandList *cmdList, std::vector<MeshData> meshData);
    void Destroy();

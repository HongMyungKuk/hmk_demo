...
namespace Graphics
{
	...
	std::vector<D3D12_INPUT_ELEMENT_DESC> samplingILDesc;
	...
	void InitShader()
	{
		...
		samplingILDesc = { {"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
			   {"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
			   {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
			   {"TANGENT", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 32, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0} };
	}
	...
	void InitRootSignature(ID3D12Device* device)
	{
		...
		}
		{
			...
			CD3DX12_DESCRIPTOR_RANGE rangeObj2[1] = {};
			rangeObj2[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);

			D3D12_ROOT_SIGNATURE_FLAGS rootSignatureFlags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;

			CD3DX12_ROOT_PARAMETER rootParameters[3] = {};
			rootParameters[0].InitAsDescriptorTable(1, rangeObj1);
			rootParameters[1].InitAsDescriptorTable(1, rangeObj2);
			rootParameters[2].InitAsConstantBufferView(0);

			CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc;
			rootSignatureDesc.Init(_countof(rootParameters), rootParameters, 1, &Graphics::linearClampSD,
				rootSignatureFlags);
	...
	void InitPipeLineState(ID3D12Device* device)
	{
		...
		psoDesc.SampleDesc.Count = 1;
		psoDesc.SampleDesc.Quality = 0;
		ThrowIfFailed(device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&postEffectsPSO)));

...
void KbmInitialize()
{
    ...
    ThrowIfFailed(s_keyboard->SetCooperativeLevel(EngineCore::g_hwnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE));
...
void KbmUpdate()
{
    ...
    if (foreground != EngineCore::g_hwnd || !visible) // wouldn't be able to acquire
    {
        KbmZeroInputs();
    }
    else
